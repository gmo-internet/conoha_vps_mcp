...
その他: `npm run dev`（開発サーバー）, `npm run dev:ui`（MCP App UIをサンプルデータでプレビュー、認証情報不要）, `npm run inspector`（MCPインスペクター）, `npm run generate:notice`（NOTICE生成）, `npm run docs:build`（APIドキュメント生成）
